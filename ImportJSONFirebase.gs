...
function ImportJSON(jsonData) {
  return parseJSONObject_(jsonData);
}
...
function parseJSONObject_(object) {
  ...
  parseData_(headers, data, "", 1, object);
 ...
 */
function parseData_(headers, data, path, rowIndex, value) {
  var dataInserted = false;
  
  if (isObject_(value)) {
    for (key in value) {
      if (parseData_(headers, data, key, rowIndex, value[key])) {
        dataInserted = true; 
      }
    }
  } else if (Array.isArray(value) && isObjectArray_(value)) {
    for (var i = 0; i < value.length; i++) {
      if (parseData_(headers, data, path, rowIndex, value[i])) {
        dataInserted = true;
        rowIndex++;
      }
    }
